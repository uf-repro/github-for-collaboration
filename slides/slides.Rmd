...
* There are many ways to collaborate using Git... not all of these are suitable for small research teams!
...
* `git merge` combines the changes from diverging commits:
  ...
  - A makes changes & commits `{a}`
  - B makes changes & commits `{b}`
...
* when git cannot combine the changes automatically, you must tell it what the files should be in the merge commit.
* for a text file:
...
* After the conflicts are resolved, you can proceed with the merge and create the merged commit.
...
* When you want multiple sequences:
  - create a new branch
  - merge when ready  
